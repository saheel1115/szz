...
class DatabaseCon:
  def __init__(self, db, dbUser, dbHost, dbPort):
    ...
    self.conn = psycopg2.connect(database=db, user=dbUser, host=dbHost, port=dbPort,password="bairay")
  ...
  def insert(self, sql_command):
    ...
    try:
      ...
      print("!! Error inserting command %s" % (sql_command))
      raise
  ...
  def execute(self, sql_command):
    ...
    try:
      ...
      print("!! Error executing command %s" % (sql_command))
      raise
      return None
  ...
  def test(config_file):
    ...
    dbCon = DatabaseCon(db_config['database'], db_config['user'], db_config['host'], db_config['port'])
