...
void Suggestion::DealFile(const string& input_file)
{
    ...
    while (getline(fin, line))
    {
        ...
        {
            GetLastNWords(history, Data::NGRAM_ORDER-1, prefix);

            if (Data::USE_CACHE)
            {
                // generate new prefix for cache
                GetLastNWords(history, Data::CACHE_ORDER-1, cache_prefix);
            }
    ...
    if (Data::USE_CACHE)
    {
        ...
        if (cache_count != 0)
        {
            ...
            */
            ...
            {
                if (Data::CACHE_BACKOFF_WEIGHT != 0)
                {
                    if (Data::CACHE_BACKOFF_WEIGHT == -1)
                    {
                        //  decrease weight when backing off
                        float smooth_discount = float(valid_order)/Data::NGRAM_ORDER;
                        cache_discount *= smooth_discount;
                    }
                    else
                    {
                        float increase_space = 1 - cache_discount;
                        float smooth_discount = increase_space/Data::NGRAM_ORDER*(Data::NGRAM_ORDER-valid_order);
                        cache_discount *= smooth_discount;
                    }
                }
            }
            ...
            */
            ...
            string valid_cache_prefix;
            GetLastNWords(cache_prefix, valid_order-1, valid_cache_prefix);
            int cache_ngram_count = Data::CACHE.GetCount(valid_cache_prefix, token);
            prob = ngram_discount * pow(2, prob) + cache_discount * ((float)cache_ngram_count/cache_count);
            debug_info += ", in cache: " + to_string1(cache_ngram_count) + "/" + to_string1(cache_count);
