...
public:
    ...
    void SetSmooth(const bool smooth, const int ngram_order)
    {
        m_smooth = smooth;
        m_ngram_order = ngram_order;
    }
    /**
    ...
    **/
    ...
    void Build(const string& input_file, const int cache_order, const bool use_type, const bool on_lexeme)
    {
        ...
        for (int i=m_min_order-1; i<(int)tokens.size(); i++)
        {
            ...
            Join(tokens, start, end, cache_prefix);
    ...
    **/
    ...
    void UpdateCandidates(const string& prefix, const float cache_lambda, const bool cache_dynamic_lambda, vector<Word>& candidates)
    {
        ...
        if (cache_count != 0)
        {
            ...
            if (m_smooth && m_ngram_order>1)
            {
                ...
                {
                    smooth_discount *= (float)(m_ngram_order-1)/m_ngram_order;
                }
            ...
            for (int i=0; i<(int)candidates.size(); ++i)
            {
                ...
                if (iter != token_counts.end())
                {
                    ...
                    candidates.at(i).m_debug += ", in cache with order " + to_string1(valid_order) + " : " + to_string1(iter->second) + "/" + to_string1(cache_count);
            ...
            for (iter=token_counts.begin(); iter!=token_counts.end(); ++iter)
            {
                ...
                candidate.m_debug += "only in cache with order " + to_string1(valid_order) + " : " + to_string1(iter->second) + "/" + to_string1(cache_count);
    ...
    int GetValidOrder(const string& prefix, const string& token)
    {
        int n = CountWords(prefix);
        string new_prefix;
        for (int i=n; i>=m_min_order-1; --i)
    ...
    int GetCount(const string& prefix, const string& token)
    {
        ...
        */
        ...
        map<string, Record>::iterator iter = m_records.find(prefix);
        if (iter != m_records.end())
        {
            int count = iter->second.Find(token);
